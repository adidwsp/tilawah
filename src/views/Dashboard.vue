    ...
    <div class="card bg-white transform hover:scale-[1.02] transition-all duration-200">
          ...
          <div class="h-48 relative">
            ...
            <div class="absolute inset-0 flex items-end px-2">
              ...
              >
                <!-- Bar -->
                <div 
                  class="w-full rounded-t-lg transition-all duration-300 cursor-pointer relative z-10"
                  :class="day.active ? 'bg-primary' : 'bg-gray-200'"
                  :style="{ height: day.height + '%' }"
                  :title="`${day.day}: ${day.juz} Juz`"
                >
                  <!-- Hover Effect -->
                  <div class="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity rounded-t-lg"></div>
                </div>
                ...
                <div 
                  class="absolute top-0 w-2 h-2 bg-white border-2 rounded-full transform -translate-y-1/2 z-20"
                  :class="day.active ? 'border-primary' : 'border-gray-300'"
                  :style="{ 
      ...
      <div class="fixed bottom-20 left-0 right-0 px-4 z-10">
        <div class="bg-white rounded-2xl shadow-lg p-4">
          <button 
            @click="goToReport"
            class="btn-primary w-full flex items-center justify-center gap-2 px-6 py-3"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
